...
class NetworkArticleRepository(private val apiKey: String) : ArticleRepository {
    ...
    override suspend fun getTopHeadlines(): Result<List<Article>> {
        return try {
            val response = RetrofitClient.newsApi.getTopHeadlines(country = "us", apiKey = apiKey)
    ...
    override suspend fun getEverything(query: String): Result<List<Article>> {
        return try {
            val response = RetrofitClient.newsApi.getEverything(query = query, from = null, sortBy = "popularity", apiKey = apiKey)
            if (response.isSuccessful) {
                Result.Success(response.body()?.articles ?: emptyList())
            } else {
                Result.Error("Ошибка API: ${response.code()}")
